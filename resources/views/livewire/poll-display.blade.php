<div class="space-y-5">
        ...
        <div class="p-5">
            ...
            <div class="mt-4 grid grid-cols-4 gap-px overflow-hidden rounded-lg border border-gray-100 bg-gray-100 dark:border-gray-700 dark:bg-gray-700">
                <div class="bg-white px-3 py-2.5 text-center dark:bg-gray-800">
                    ...
                    <p class="text-[10px] font-medium uppercase tracking-wider text-gray-400">{{ __('poller::messages.total_votes') }}</p>
                </div>
                <div class="bg-white px-3 py-2.5 text-center dark:bg-gray-800">
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $uniqueVoters }}</p>
                    <p class="text-[10px] font-medium uppercase tracking-wider text-gray-400">{{ __('poller::messages.voters') }}</p>
                </div>
                <div class="bg-white px-3 py-2.5 text-center dark:bg-gray-800">
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $poll->options->count() }}</p>
                    <p class="text-[10px] font-medium uppercase tracking-wider text-gray-400">{{ __('poller::messages.options_count') }}</p>
                </div>
                <div class="bg-white px-3 py-2.5 text-center dark:bg-gray-800">
                    ...
                    @endif
                    <p class="text-[10px] font-medium uppercase tracking-wider text-gray-400">{{ $poll->ends_at ? __('poller::messages.ends') : __('poller::messages.no_deadline') }}</p>
                </div>
            </div>
        </div>
        ...
        @if($poll->is_anonymous || $poll->allow_vote_change || $poll->requires_comment || $poll->show_results_before_close)
            <div class="flex flex-wrap gap-1.5 border-t border-gray-100 px-5 py-3 dark:border-gray-700">
                @if($poll->is_anonymous)
                    <span class="rounded-md bg-purple-50 px-2 py-0.5 text-[10px] font-medium text-purple-600 dark:bg-purple-900/20 dark:text-purple-400">{{ __('poller::messages.anonymous') }}</span>
                @endif
                @if($poll->allow_vote_change)
                    <span class="rounded-md bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-600 dark:bg-amber-900/20 dark:text-amber-400">{{ __('poller::messages.vote_change_allowed') }}</span>
                @endif
                @if($poll->requires_comment)
                    <span class="rounded-md bg-sky-50 px-2 py-0.5 text-[10px] font-medium text-sky-600 dark:bg-sky-900/20 dark:text-sky-400">{{ __('poller::messages.comment_required_badge') }}</span>
                @endif
                @if($poll->show_results_before_close)
                    <span class="rounded-md bg-teal-50 px-2 py-0.5 text-[10px] font-medium text-teal-600 dark:bg-teal-900/20 dark:text-teal-400">{{ __('poller::messages.live_results') }}</span>
                @endif
            </div>
        @endif
    </div>
    ...
    <div class="flex gap-1 rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
        @foreach(['overview' => __('poller::messages.overview'), 'results' => __('poller::messages.results'), 'votes' => __('poller::messages.votes_tab')] as $tab => $label)
    ...
    @if($activeTab === 'overview')
        <livewire:poller-poll-vote :poll="$poll" :key="'vote-' . $poll->id" />
    @elseif($activeTab === 'results')
        @if($canShowResults)
            <livewire:poller-poll-results :poll="$poll" :key="'results-' . $poll->id" />
        @else
            <div class="rounded-xl border border-dashed border-gray-200 px-4 py-12 text-center dark:border-gray-700">
                <p class="text-xs text-gray-400">{{ __('poller::messages.results_available_after_close') }}</p>
            </div>
        @endif
    @elseif($activeTab === 'votes')
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-700">
                            <th class="px-4 py-2.5 text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('poller::messages.voter') }}</th>
                            <th class="px-4 py-2.5 text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('poller::messages.option_column') }}</th>
                            <th class="px-4 py-2.5 text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('poller::messages.comment') }}</th>
                            <th class="px-4 py-2.5 text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('poller::messages.date') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                        @foreach($poll->votes()->with(['user', 'option'])->latest()->get() as $vote)
                            <tr class="transition hover:bg-gray-50/50 dark:hover:bg-gray-750">
                                <td class="px-4 py-2.5 text-sm text-gray-900 dark:text-white">{{ $vote->user?->name ?? __('poller::messages.unknown') }}</td>
        ...
        @else
            <div class="rounded-xl border border-dashed border-gray-200 px-4 py-12 text-center dark:border-gray-700">
                <p class="text-xs text-gray-400">{{ __('poller::messages.anonymous_votes_hidden') }}</p>
            </div>
        @endif
    @endif
</div>
