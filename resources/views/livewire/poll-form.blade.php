<div class="space-y-5">
    <div class="flex items-center justify-between">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">
            {{ $pollId ? __('poller::messages.edit_poll') : __('poller::messages.create_poll') }}
        </h3>
    ...
    <form wire:submit="save" class="space-y-5">
        {{-- Title --}}
        <div>
            <label for="title" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.title') }} <span class="text-red-400">*</span></label>
            <input wire:model="title" type="text" id="title"
                ...
                placeholder="{{ __('poller::messages.enter_poll_title') }}">
            @error('title') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
        </div>

        {{-- Description --}}
        <div>
            <label for="description" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.description') }}</label>
            <textarea wire:model="description" id="description" rows="2"
                class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 placeholder-gray-400 transition focus:border-gray-300 focus:bg-white focus:ring-0 dark:border-gray-600 dark:bg-gray-700/50 dark:text-white dark:focus:border-gray-500 dark:focus:bg-gray-700"
                placeholder="{{ __('poller::messages.optional_description') }}"></textarea>
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label for="type" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.type') }} <span class="text-red-400">*</span></label>
            ...
            <div>
                <label for="status" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.status') }} <span class="text-red-400">*</span></label>
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label for="starts_at" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.start_date') }}</label>
            ...
            <div>
                <label for="ends_at" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.end_date') }}</label>
        ...
        <div class="grid grid-cols-3 gap-3">
            <div>
                <label for="min_selections" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.min_selections') }}</label>
            ...
            <div>
                <label for="max_selections" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.max_selections') }}</label>
            ...
            <div>
                <label for="max_votes_per_user" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.max_votes_per_user') }}</label>
        ...
        <div class="flex flex-wrap gap-x-5 gap-y-2.5">
            @foreach([
                'is_anonymous' => __('poller::messages.anonymous_voting'),
                'show_results_before_close' => __('poller::messages.show_results_before_close'),
                'allow_vote_change' => __('poller::messages.allow_vote_change'),
                'requires_comment' => __('poller::messages.require_comment_with_vote'),
            ] as $field => $label)
            ...
            <label class="inline-flex cursor-pointer items-center gap-2">
                <input wire:model.live="allow_custom_options" type="checkbox"
                    class="size-3.5 rounded border-gray-300 text-gray-900 transition focus:ring-0 focus:ring-offset-0 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                <span class="text-xs text-gray-600 dark:text-gray-400">{{ __('poller::messages.allow_custom_options') }}</span>
            </label>
        </div>
        ...
        @if($allow_custom_options)
            <div class="max-w-48">
                <label for="max_custom_options" class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.max_custom_options') }}</label>
        ...
        <div>
            <div class="mb-2 flex items-center justify-between">
                <label class="text-xs font-medium text-gray-600 dark:text-gray-400">{{ __('poller::messages.options_label') }} <span class="text-red-400">*</span></label>
                    ...
                    <div wire:key="option-{{ $index }}" class="flex items-start gap-2">
                        ...
                        <div class="flex-1 space-y-1.5">
                            <input wire:model="options.{{ $index }}.title" type="text"
                                ...
                                placeholder="{{ __('poller::messages.option') }} {{ $index + 1 }}">
                            ...
                            <input wire:model="options.{{ $index }}.description" type="text"
                                ...
                                placeholder="{{ __('poller::messages.description_optional') }}">
                        </div>
                        @if(count($options) > 2)
        ...
        <div class="flex items-center justify-end gap-2 border-t border-gray-100 pt-4 dark:border-gray-700">
            ...
            <button type="submit"
                ...
                {{ $pollId ? __('poller::messages.update_poll') : __('poller::messages.create_poll') }}
            </button>
        </div>
    </form>
</div>
