<div class="space-y-3">
    ...
    <div class="flex items-center justify-between">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('poll-vote::messages.results') }}</h3>
        <div class="flex items-center gap-3 text-xs text-gray-400">
            <span>{{ $totalVotes }} {{ __('poll-vote::messages.total_votes_label') }}</span>
            <span class="text-gray-200 dark:text-gray-600">&middot;</span>
            <span>{{ $uniqueVoters }} {{ __('poll-vote::messages.voters_label') }}</span>
        </div>
    </div>
        ...
        @if($leadingOption)
            <div class="flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 dark:bg-emerald-900/10">
                ...
                <p class="text-xs text-emerald-700 dark:text-emerald-400">
                    <span class="font-semibold">{{ __('poll-vote::messages.leading') }}:</span> {{ $leadingOption->title }}
                    <span class="text-emerald-500">({{ $leadingOption->votes_count }} {{ __('poll-vote::messages.votes') }})</span>
                </p>
            </div>
        @endif
    @else
        <div class="rounded-xl border border-dashed border-gray-200 px-4 py-12 text-center dark:border-gray-700">
            <p class="text-xs text-gray-400">{{ __('poll-vote::messages.no_votes_yet') }}</p>
        </div>
    @endif
</div>
