...
it('votes on a poll via service helper', function () {
    ...
    $votes = app('larapoll')->castVote($poll, $this->user, $option->id);
...
it('changes vote via service helper', function () {
    ...
    $newVotes = app('larapoll')->changeVote($poll, $this->user, $optB->id);
...
it('gets user voting history', function () {
    ...
    app('larapoll')->castVote($poll1, $this->user, $opt1->id);
    app('larapoll')->castVote($poll2, $this->user, $opt2->id);

    $history = app('larapoll')->getUserVotingHistory($this->user);
...
it('limits user voting history', function () {
    for ($i = 0; $i < 5; $i++) {
        ...
        app('larapoll')->castVote($poll, $this->user, $opt->id);
    }

    $history = app('larapoll')->getUserVotingHistory($this->user, 3);
