...
it('enforces max_votes_per_user limit', function () {
    ...
    app('poller')->castVote($poll, $voter2, $option->id);
...
it('accepts vote with comment when requires_comment is enabled', function () {
    ...
    $votes = app('poller')->castVote($poll, $this->user, $option->id, ['comment' => 'My comment']);
...
it('rejects rating outside configured range', function () {
    ...
    app('poller')->castVote($poll, $this->user, $option->id, ['rating' => 10]);
...
it('accepts rating within configured range', function () {
    ...
    $votes = app('poller')->castVote($poll, $this->user, $option->id, ['rating' => 3]);

    expect($votes->first()->rating)->toBe(3);
});

it('respects custom rating range from config', function () {
    config()->set('poller.rating.min', 1);
    config()->set('poller.rating.max', 10);
    ...
    $votes = app('poller')->castVote($poll, $this->user, $option->id, ['rating' => 8]);
...
it('prevents type change on poll with votes', function () {
    ...
    app('poller')->update($poll, ['type' => 'multiple_choice']);
...
it('allows type change on poll without votes', function () {
    ...
    $updated = app('poller')->update($poll, ['type' => 'multiple_choice']);
...
it('rejects too many selections on multiple choice', function () {
    ...
    app('poller')->castVote($poll, $this->user, [$opt1->id, $opt2->id, $opt3->id]);
...
it('rejects options from different poll', function () {
    ...
    app('poller')->castVote($poll, $this->user, $foreignOption->id);
...
it('requires comment on vote change when poll requires comment', function () {
    ...
    app('poller')->castVote($poll, $this->user, $optA->id, ['comment' => 'First comment']);
...
it('casts ranked votes with rank values', function () {
    ...
    $votes = app('poller')->castVote($poll, $this->user, [$opt1->id, $opt2->id], [
        'ranks' => [1, 2],
    ]);
