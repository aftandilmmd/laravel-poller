...
it('casts a vote on single choice poll', function () {
    $votes = app('poller')->castVote($this->poll, $this->user, $this->optionA->id);
...
it('prevents duplicate voting', function () {
    ...
    app('poller')->castVote($this->poll, $this->user, $this->optionB->id);
...
it('prevents multiple selections on single choice poll', function () {
    app('poller')->castVote($this->poll, $this->user, [$this->optionA->id, $this->optionB->id]);
...
it('allows multiple selections on multiple choice poll', function () {
    ...
    $votes = app('poller')->castVote($poll, $this->user, [$optA->id, $optB->id]);
...
it('changes a vote when allowed', function () {
    ...
    $newVotes = app('poller')->changeVote($this->poll, $this->user, $this->optionB->id);
...
it('retracts a vote', function () {
    ...
    app('poller')->retractVote($this->poll, $this->user);
