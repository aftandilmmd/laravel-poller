...
it('adds a custom option to a poll', function () {
    $option = app('poller')->addCustomOption($this->poll, $this->user, ['title' => 'My Custom Option']);
...
it('prevents custom options when not allowed on poll', function () {
    ...
    app('poller')->addCustomOption($poll, $this->user, ['title' => 'Should fail']);
...
it('prevents custom options when feature is disabled globally', function () {
    config()->set('poller.features.custom_options', false);

    app('poller')->addCustomOption($this->poll, $this->user, ['title' => 'Should fail']);
})->throws(CustomOptionException::class, 'Custom options are not allowed for this poll.');
...
it('enforces max custom options limit', function () {
    ...
    app('poller')->addCustomOption($poll, $this->user, ['title' => 'Custom 1']);
    app('poller')->addCustomOption($poll, $this->user, ['title' => 'Custom 2']);
    app('poller')->addCustomOption($poll, $this->user, ['title' => 'Custom 3']);
...
it('allows unlimited custom options when max is null', function () {
    ...
    for ($i = 1; $i <= 10; $i++) {
        app('poller')->addCustomOption($poll, $this->user, ['title' => "Custom {$i}"]);
    }
...
it('auto assigns sort order to custom options', function () {
    $option = app('poller')->addCustomOption($this->poll, $this->user, ['title' => 'Custom']);
